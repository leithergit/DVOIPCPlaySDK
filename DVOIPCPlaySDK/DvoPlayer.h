...
private:
	...
	int GetLastFrameID(int &nLastFrameID)
	{
		...
		if (nOffset < 0)
		{
			...
			if (nOffset < 0)
				return DVO_Error_MaxFrameSizeNotEnough;
			else if (nOffset < offsetof(DVOFrameHeader, nFrameTag))
			{
				pBuffer += (nOffset + 4);
...
//#endif
		...
		while (true && bWorking)
		{
			...
			while (true && bWorking)
			{
				...
				{
					if (InputStream((byte *)Parser.pHeader, Parser.nFrameSize, (UINT)nMaxCache,true) == DVO_Error_FrameCacheIsFulled &&
						bWorking)
					{
...
// 					OutputMsg("Frame ID:%d\tType = Audio:%d.\n", pFrameParser->pHeaderEx->nFrameID, pFrameParser->pHeaderEx->nType);
	...
	{// 若指定了有效的窗口句柄，则把解析后的文件数据放入播放队列，否则不放入播放队列
		...
		if (pThis->GetFileSummary(pThis->m_bThreadParserRun) != DVO_Succeed)
		{
			assert(false);
			return 0;
		}
