...
enum DVO_CALLBACK_MESSAGE_TYPE
{
    ...
    DVO_MSG_ALARM_HEARTBEAT                 =  0x08,    // Ioin[0]不报警 MD[0]不报警 <=>Ioin[0]通路 MD[0]不报警
    DVO_MSG_FUNCTION_REPORT                 =  0x09,    // 像机功能上报类型
...
enum EDVO_DEVICE_PARAMETER_TYPE
{
    ...
    DVO_DEV_CMD_SYS_IOATTR_GET              = MAKEINT32(0x001,0x01F),   // IO报警端口获取,app_net_tcp_set_io_alarm_attr_t;
    DVO_DEV_CMD_SYS_IOATTR_SET              = MAKEINT32(0x001,0x020),   // IO报警端口设置

    DVO_DEV_CMD_SYS_IOUSE_SET               = MAKEINT32(0x001,0x021),   // IO报警端口使能设置,app_net_tcp_set_io_alarm_state_t;
    DVO_DEV_CMD_SYS_IOUSE_GET               = MAKEINT32(0x001,0x022),   // IO报警端口使能查询
